...
use PhpOffice\PhpSpreadsheet\Style\{Font, Border, Alignment};
    class Excelmil
    {
        ...
        private function table_header_spec_print($infspec)
        {
                ...
                $this->sheet->getStyle("A".($this->strokaExcel)."".":".$this->bukvaColum[$this->shirinatable]."".($this->strokaExcel))->getFont()->setSize(15);
        ...
        private function table_header_spec_group_print($infGroup)
        {
            $v=$this->strokaExcel;	
                    ...
                    $this->sheet->setCellValueByColumnAndRow($columnExcel+1, $this->strokaExcel, 'Дисциплины ВП');
                    $this->sheet->mergeCells($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[$this->shirinatable]."".($this->strokaExcel));
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel)."".":".$this->bukvaColum[$this->shirinatable]."".($this->strokaExcel))->applyFromArray($this->border);
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel)."".":".$this->bukvaColum[$this->shirinatable]."".($this->strokaExcel))->getAlignment()->setHorizontal('center');  
                    $this->strokaExcel++;
                    $columnExcel=1;
                    ...
                    $this->sheet->setCellValueByColumnAndRow($columnExcel+1, $this->strokaExcel, 'Экзамен');
                    $this->sheet->mergeCells($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel));
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel))->applyFromArray($this->border);
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel))->getAlignment()->setHorizontal('center');   
                    $columnExcel=$columnExcel+$this->MaxKolExamVP;
                    $this->sheet->setCellValueByColumnAndRow($columnExcel+1, $this->strokaExcel, 'Зачеты');
                    $this->sheet->mergeCells($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel));
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel))->applyFromArray($this->border);
                    $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel))->getAlignment()->setHorizontal('center');   
                    ...
                    foreach($predmet as $IDEPC => $infpred)
                    {
                        switch ($Typecontr)
                        {
                            case 'exam':
                                ...
                                {
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel, (string)$infpred['edu_name']); 
                                    $columnExcelExamVP++;
                                }
                                break;
                            case 'zachet':
                            case 'difzachet':
                                ...
                                {
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, (string)$infpred['edu_name']); 
                                    $columnExcelZachetVP++;
                                }
                                break;
                        }
        ...
        private function table_stat_information_group_print($infGroup)
        {
            ...
            $columnExcel=$columnExcel+$this->MaxKolZachetOOP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+1).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel+4))->applyFromArray($this->border);
            $columnExcel=$columnExcel+$this->MaxKolExamVP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+1).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel+4))->applyFromArray($this->border);
            ...
            $columnExcel=$columnExcel+$this->MaxKolZachetOOP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+5).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel+5))->applyFromArray($this->border);
            $columnExcel=$columnExcel+$this->MaxKolExamVP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+5).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel+5))->applyFromArray($this->border);
            ...
            $columnExcel=$columnExcel+$this->MaxKolZachetOOP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+6).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel+7))->applyFromArray($this->border);
            $columnExcel=$columnExcel+$this->MaxKolExamVP;
            $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($this->strokaExcel+6).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel+7))->applyFromArray($this->border);
                    ...
                    foreach($predmet as $IDEPC => $infpred)
                    {
                        switch ($Typecontr)
                        {
                            case 'exam':
                                ...
                                {                                    
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel, $infpred['avg']);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel+1, $infpred['marks'][5]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel+2, $infpred['marks'][4]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel+3, $infpred['marks'][3]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel+4, $infpred['marks'][2]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel+5, $infpred['marks'][1]);   
                                    $columnExcelExamVP++;
                                }
                                break;
                            case 'zachet':
                                ...
                                {
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+6, $infpred['marks'][5]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+7, $infpred['marks'][1]);
                                    $columnExcelZachetVP++;
                                }
                                break;
                            case 'difzachet':
                                ...
                                {
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, $infpred['avg']);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+1, $infpred['marks'][5]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+2, $infpred['marks'][4]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+3, $infpred['marks'][3]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+4, $infpred['marks'][2]);
                                    $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel+5, $infpred['marks'][1]); 
                                    $columnExcelZachetVP++;
                                }
                                break;
                        }                        
        ...
        private function table_student_information_group_print($grnum)
        {
                        ...
                        switch ($Typecontr)
                        {
                            case 'exam':
                                ...
                                else
                                {
                                    if ($infpred['mark']!=1)
                                    {
                                        $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel, $infpred['mark']);
                                    }
                                    else
                                    {
                                        $this->sheet->setCellValueByColumnAndRow($columnExcelExamVP, $this->strokaExcel, 'Не явка'); 
                                    }                                                                          
                                    $columnExcelExamVP++;
                                }
                                break;
                            case 'zachet':
                                ...
                                else
                                {
                                    if($infpred['mark']=='7')
                                            {											
                                                $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, "зачет");			
                                            }
                                            else
                                            {											
                                                $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, "не зачет");									
                                            }
                                    $columnExcelZachetVP++;
                                }
                                break;
                            case 'difzachet':
                                ...
                                else
                                {
                                    if ($infpred['mark']!=1)
                                    {
                                        $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, $infpred['mark']);
                                    }
                                    else
                                    {
                                        $this->sheet->setCellValueByColumnAndRow($columnExcelZachetVP, $this->strokaExcel, 'Не явка'); 
                                    } 
            ...
            $columnExcel=1;
                ...
                $columnExcel=$columnExcel+$this->MaxKolZachetOOP;
                $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($firstExcelstroka).":".$this->bukvaColum[($columnExcel+$this->MaxKolExamVP-1)]."".($this->strokaExcel))->applyFromArray($this->border);
                $columnExcel=$columnExcel+$this->MaxKolExamVP;
                $this->sheet->getStyle($this->bukvaColum[$columnExcel]."".($firstExcelstroka).":".$this->bukvaColum[($columnExcel+$this->MaxKolZachetVP-1)]."".($this->strokaExcel))->applyFromArray($this->border);
        ...
        private function excelgenerator()
        { 
